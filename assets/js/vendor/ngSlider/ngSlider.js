(function($) {
  ...
  function testAnimate(parsed, entry) {
    ...
    if (typeof entry !== 'boolean') {
      throw new Error("slider: 'animate' option must be a boolean.");
    }
  }
  ...
  function testConnect(parsed, entry) {
    ...
    if (entry === 'lower' && parsed.handles === 1) {
      ...
      throw new Error("slider: 'connect' option doesn't match handle count.");
    }
  }
  ...
  function testOrientation(parsed, entry) {
    ...
    switch (entry) {
      ...
      case 'vertical':
        ...
        break;
      default:
        throw new Error("slider: 'orientation' option is invalid.");
    }
  }
  ...
  function testLimit(parsed, entry) {
    ...
    if (!parsed.limit) {
      throw new Error("slider: 'limit' option is only supported on linear sliders.");
    }
  }
  ...
  function testDirection(parsed, entry) {
    ...
    switch (entry) {
      ...
      case 'rtl':
        ...
        break;
      default:
        throw new Error("slider: 'direction' option was not recognized.");
    }
  }
  ...
  function fixEvent(e) {
    ...
    if (touch) {
      // slider supports one movement at a time,
  ...
  function closure(target, options, originalOptions) {
    ...
    function attach(events, element, callback, data) {
      ...
      // Add the slider namespace to all events.
    ...
    function valueSet(input) {
      ...
      // Fire the 'set' event. As of slider 7,
  ...
  // Extend jQuery/Zepto with the slider method.
  $.fn.slider = function(options, rebuildFlag) {
  ...
  .directive('slider', function() {
    return {
      ...
      link: function(scope, element, attrs) {
        var callback, fromParsed, parsedValue, $element, toParsed;
        $element = $(element);
        $element.addClass('slider-' + scope.type);
        ...
        if (scope.ngFrom != null && scope.ngTo != null) {
          ...
          $element.on(callback, function() {
            var from, to, _ref;
            _ref = $element.val(), from = _ref[0], to = _ref[1];
          ...
          $element.on(callback, function() {
            parsedValue = parseFloat($element.val());
            return scope.$apply(function() {
