            ...
            <div class="col-xs-12">
              <div class="card">
                ...
                <div class="card-body">
                    <!-- IISI -->
                    <center>
                        <a href="<?= base_url(); ?>laporan/cetak_program_kuisioner_c_rekrut/<?= $program; ?>" target="_blank" class="btn btn-danger icon icon-file-pdf-o"> PDF</a> | <a href="<?= base_url(); ?>laporan/export_program_excel_kuisioner_c_rekrut/<?= $program; ?>" class="btn btn-success icon icon-file-excel-o"> Excel</a>
                    </center>
                    <br><br>
                    ...
                    <table class="table table-striped table-bordered">
                      <thead>
                        <tr>
                          ...
                          <th colspan="<?= $jml_kuisioner_c_rekrut;?>" class="text-center">Rekruitmen, Perjalanan, Persyaratan Peserta</th>
                          <th rowspan="2"><center> ID Peserta </center></th>
                        </tr>

                        <tr>
                        <?php 
                               $soal=1;
                              foreach ($kuisioner_c_rekrut as $key) { ?>
                      ...
                      <?php foreach($responden as $r){ ?>
                          ...
                          foreach($soal as $s){
                            ...
                            $id_soal = $s['id_soalC']; 
                            $nilainya = $this->db->query("SELECT * FROM penilaian_c INNER JOIN kuisioner_c ON id_soalC=id_kuisionerC WHERE id_user='$id_user' AND id_soalC='$id_soal' AND kd_pelatihan='$kd_pelatihan' AND jenis_soal=1 AND tipe_soal='pg'")->row_array();  
                            // 
                          ?>
                        ...
                        <tr>
                          <td>Jumlah</td>
                          <?php foreach($kuisioner_c_rekrut as $sl){ 
                        ...
                        <tr>
                          <td>NRR X Bobot</td>
                          <?php  $jmlh_keseluruhan = 0; foreach($kuisioner_c_rekrut as $sl){ 
                              $id_soalnya = $sl['id_kuisionerC'];
                              $total = $this->db->query("SELECT AVG(jawaban) as total FROM penilaian_c LEFT JOIN pelatihan ON penilaian_c.kd_pelatihan=pelatihan.kd_pelatihan 
                                                            WHERE penilaian_c.id_soalC='$id_soalnya' AND pelatihan.id_program='$program'")->row_array();
                          ?>
                            <td class="text-center"><?= number_format($total['total']/$jml_kuisioner_c_rekrut,2); ?></td>
                          <?php $jmlh_keseluruhan = $jmlh_keseluruhan+(number_format($total['total']/$jml_kuisioner_c_rekrut,2)); } ?>
                        
                        </tr>
                        <tr>
                          <td>Jumlah</td>
                          <td colspan="<?= $jml_kuisioner_c_rekrut;?>" class="text-center"><h4><?= number_format($jmlh_keseluruhan,2) ;?></h4></td>
                        </tr>
                        <tr>
                          <td>Jumlah X 25</td>
                          <td colspan="<?= $jml_kuisioner_c_rekrut;?>" class="text-center"><h4><?= $hasil_akhir= number_format($jmlh_keseluruhan*25,2) ;?>
