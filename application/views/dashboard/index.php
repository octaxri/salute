<div class="layout-content">
        <div class="layout-content-body">
          <div class="title-bar">
            <h1 class="title-bar-title">
              <span class="d-ib">Dashboard</span>
              <span class="d-ib">
                <a class="title-bar-shortcut" href="#" title="Add to shortcut list" data-container="body" data-toggle-text="Remove from shortcut list" data-trigger="hover" data-placement="right" data-toggle="tooltip">
                  <span class="sr-only">Add to shortcut list</span>
                </a>
              </span>
            </h1>
            <br>
            <!--Data jumlah jawaban masuk -->
            <div class="col-md-6 col-lg-3 col-lg-push-0">
              <div class="card">
                <div class="card-body">
                  <div class="media">
                    <div class="media-middle media-left">
                      <span class="bg-primary circle sq-48">
                        <span class="icon icon-inbox"></span>
                      </span>
                    </div>
                    <div class="media-middle media-body">
                      <h6 class="media-heading">Jawaban Masuk</h6>
                      <h3 class="media-heading">
                        <span class="fw-l"><?= $jumlah_pesan; ?></span>
                      </h3>
                    </div>
                  </div>
                </div>
              </div>
            </div>
             <!-- Akhir Data jumlah jawaban masuk -->

               <!--Data Total Peserta -->
            <div class="col-md-6 col-lg-3 col-lg-push-0">
              <div class="card">
                <div class="card-body">
                  <div class="media">
                    <div class="media-middle media-left">
                      <span class="bg-primary circle sq-48">
                        <span class="icon icon-user"></span>
                      </span>
                    </div>
                    <div class="media-middle media-body">
                      <h6 class="media-heading">Total Peserta</h6>
                      <h3 class="media-heading">
                        <span class="fw-l"><?= $jumlah_peserta; ?></span>
                      </h3>
                    </div>
                  </div>
                </div>
              </div>
            </div>
             <!-- Akhir Data Total Peserta -->

                    <!--Data Total Pengajar -->
            <div class="col-md-6 col-lg-3 col-lg-push-0">
              <div class="card">
                <div class="card-body">
                  <div class="media">
                    <div class="media-middle media-left">
                      <span class="bg-primary circle sq-48">
                        <span class="icon icon-male"></span>
                      </span>
                    </div>
                    <div class="media-middle media-body">
                      <h6 class="media-heading">Total Pengajar</h6>
                      <h3 class="media-heading">
                        <span class="fw-l"><?= $jumlah_pengajar; ?></span>
                      </h3>
                    </div>
                  </div>
                </div>
              </div>
            </div>
             <!-- Akhir Data Total Pengajar-->

                    <!--Data Total Pelatihan -->
            <div class="col-md-6 col-lg-3 col-lg-push-0">
              <div class="card">
                <div class="card-body">
                  <div class="media">
                    <div class="media-middle media-left">
                      <span class="bg-primary circle sq-48">
                        <span class="icon icon-wrench"></span>
                      </span>
                    </div>
                    <div class="media-middle media-body">
                      <h6 class="media-heading">Total Pelatihan</h6>
                      <h3 class="media-heading">
                        <span class="fw-l"><?= $jumlah_pelatihan; ?></span>
                      </h3>
                    </div>
                  </div>
                </div>
              </div>
            </div>
             <!-- Akhir Data Total Pelatihan -->


          </div>
        </div>
      </div>