        ...
        <?php } ?> 
          ...
          <div class="title-bar">
            <h1 class="title-bar-title">
              <span class="d-ib"><a class="btn btn-info" href="<?= base_url(); ?>pelatihan/detail_pelatihan2/5/<?= $kd_pelatihan; ?>"><span class="icon icon-backward"></span></a> DETAIL KUISIONER C REKRUITMEN,PERJALANAN,PERSYARATAN PESERTA | Kejuruan <?= $data1['nama_kejuruan']; ?>, Program <?= $data1['nama_program']; ?></span>
            </h1>
          </div>
          <hr>
          <br>
            ...
            <div class="col-xs-12">
              <div class="card">
                ...
                <div class="card-body">
                    <!-- IISI -->
                    <center>
                        <a href="<?= base_url(); ?>laporan/cetak_kuisioner_c_rekrut/<?= $kd_pelatihan; ?>" target="_blank" class="btn btn-danger icon icon-file-pdf-o"> PDF</a> | <a href="<?= base_url(); ?>laporan/export_exel_kuisioner_c_rekrut/<?= $kd_pelatihan; ?>" class="btn btn-success icon icon-file-excel-o"> Excel</a>
                    </center>
                    <br><br>
                    ...
                    <table class="table table-striped table-bordered">
                      <thead>
                        <tr>
                          ...
                          <?php $i=0; $jml=0; foreach ($responden as $r) { ?>
                              <?php
                              $id_user=$r['id_user'];
                              $jml=$this->db->query("SELECT DISTINCT id_soalC,jenis_soal,tipe_soal FROM penilaian_c INNER JOIN kuisioner_c ON id_soalC=id_kuisionerC WHERE id_user='$id_user' AND kd_pelatihan='$kd_pelatihan' AND jenis_soal=1 AND tipe_soal='pg'  ")->num_rows();
                              ?>
                          <?php } ?>
                          <th colspan="<?= $jml;?>" class="text-center">Rekruitmen, Perjalanan, Persyaratan Peserta</th>
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <?php 
                            $z = 1;
                            $soalnya1 = $this->db->query("SELECT DISTINCT id_soalC,jenis_soal,tipe_soal FROM penilaian_c INNER JOIN kuisioner_c ON id_soalC=id_kuisionerC WHERE kd_pelatihan='$kd_pelatihan' AND jenis_soal=1 AND tipe_soal='pg'  ")->result_array(); 
                        ...
                        <tr>
                          ...
                          <?php 
                           $jml_semua=0;
                            ...
                            $jml_soal= $this->db->query("SELECT DISTINCT id_soalC,jenis_soal,tipe_soal FROM penilaian_c INNER JOIN kuisioner_c ON id_soalC=id_kuisionerC WHERE kd_pelatihan='$kd_pelatihan' AND jenis_soal=1 AND tipe_soal='pg' ")->num_rows();
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped table-bordered">
                      ...
                      <?php $no=1; foreach($pelatihan as $pl){
                            $kd = $pl['kd_pelatihan']; 
                            $tampung = $this->db->query("SELECT * FROM penilaian_c LEFT JOIN kuisioner_c ON kuisioner_c.id_kuisionerC=penilaian_c.id_soalC WHERE kuisioner_c.jenis_soal=1 AND kuisioner_c.tipe_soal='uraian' AND penilaian_c.kd_pelatihan='$kd'")->result_array();
